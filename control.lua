...
local function on_player_used_capsule(event)
    ...
    -- Получаем поверхность корабля через поиск по стандартным именам WDM
    local ship_surface = nil
    local ship_position = nil
    local force = player.force
    
    if force and force.name then
        -- Ищем поверхность корабля по стандартным именам WDM
        -- Основная поверхность корабля обычно ship_interior_0_<force_name> или ship_interior_1_<force_name>
        for i = 0, 6 do
            ...
            if surf and surf.valid then
                ...
                if not (lost and lost.end_tick and game.tick < lost.end_tick) then
                    ship_surface = surf
                    -- Получаем позицию спавна на поверхности корабля
        ...
        -- Если не нашли через стандартные имена, пробуем найти через actual_surface из WDM storage
        if not ship_surface and remote.interfaces["WDM"] then
            local ok, result = pcall(function()
                -- Пытаемся получить информацию о корабле
                local planet_info = remote.call("WDM", "get_ship_planet_info", force.name)
                if planet_info then
                    -- Ищем любую поверхность корабля
                    for i = 0, 6 do
                        local surf_name = "ship_interior_" .. i .. "_" .. force.name
                        local surf = game.surfaces[surf_name]
                        if surf and surf.valid then
                            ...
                            if not (lost and lost.end_tick and game.tick < lost.end_tick) then
                                ship_surface = surf
                                ship_position = force.get_spawn_position(surf)
                                break
                            end
                        end
                    end
    ...
    -- Удаляем 50% инвентаря
    local inventory = player.get_inventory(defines.inventory.character_main)
    if inventory and inventory.valid then
        local items_to_remove = {}
        local total_items = 0
        
        -- Собираем все предметы через перебор слотов
        ...
        -- Удаляем 50% (округляем вниз)
        if total_items > 0 then
            local items_to_delete = math.floor(total_items / 2)
            local deleted = 0
            
            -- Удаляем случайные предметы до достижения 50%
    ...
    -- Телепортируем игрока к кораблю (ищем безопасную позицию)
    if ship_surface and ship_surface.valid and ship_position then
        local safe_pos = (find_safe_teleport_position and find_safe_teleport_position(ship_surface, ship_position)) or ship_position
        player.teleport(safe_pos, ship_surface)
        player.print({"wdm-expansion.emergency_recall_success"})
...
-- storage
-- =====================================================================
local function init_heat_pipe_storage()
...
-- sync
-- =====================================================================
function sync_linked_heat_pipes()
...
local function on_entity_built(event)
    pcall(function() on_pipe_built(event) end)
    pcall(function()
        local entity = event.entity or event.created_entity
...
local function on_entity_removed(event)
    pcall(function() on_pipe_removed(event) end)
    pcall(function() on_crystal_mined(event) end)
    pcall(function() remove_storm_disabled_record_for_entity(event.entity) end)
end

-- =====================================================================
-- registration
-- =====================================================================
local function register_heat_pipe_events()
...
-- lifecycle
-- =====================================================================
local function init_heat_pipe_system()
    init_heat_pipe_storage()
    register_heat_pipe_events()
    refresh_heat_pipe_tick()
end

script.on_init(function()
    initialize_mod()
    init_heat_pipe_system()
end)
